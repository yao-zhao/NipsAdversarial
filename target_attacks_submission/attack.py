...
if sigmoid_perturb:
    ...
    try:
        sigmoid_ext = Config.getint('model', 'sigmoid_ext')
    except:
        sigmoid_ext = 0
    try:
...
dense_labels = tf.placeholder(tf.int32, shape=[batch_size])
...
with tf.device(gpu):
    ...
    init_ops = [tf.assign(x_max, tf.clip_by_value(img + eps, -1.0, 1.0), name='xmax_init_op'),
                ...
                tf.assign(labels, tf.one_hot(dense_labels, num_classes))]
    if rand_start:
    ...
    if sigmoid_perturb:
        ...
        img_input = x + (eps + sigmoid_ext) * (tf.sigmoid(diff * slope) * 2 - 1)
        img_input = tf.clip_by_value(img_input, x_min, x_max)
        init_ops.append(tf.variables_initializer([diff], name='diff_init_op'))
    else:
        img_input = x
...
for filenames, images in util.load_images(args.input_dir, batch_shape, max_number=max_number):
    ...
    target_cls = ([all_images_taget_class[n] for n in filenames]+ [0] * (batch_size - len(filenames)))
